...
<div>
    ...
    @php
        ...
        if (!\Auth::user()->hasRole('admin')) {
            if (\Auth::user()->vendor == null || \Auth::user()->vendor->vendor_type->slug == 'parcel') {
                $showNewBtn = false;
            }
        }
    @endphp

    <x-baseview title="{{ __('Orders') }}" :showNew="$showNewBtn" link="{{ route('order.create') }}">
        ...
        <x-modal confirmText="{{ __('Update') }}" action="update" :clickAway="false">
            ...
            <div class="gap-4">
                ...
                @can('order-assign-driver')
                    <x-label for="deliveryBoyId" title="{{ __('Delivery Personal/Driver') }}">
    ...
    <div x-data="{ open: @entangle('showAssign') }">
        <x-modal confirmText="{{ __('Approve') }}" action="approvePayment" :clickAway="false">
