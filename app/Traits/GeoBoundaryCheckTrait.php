...
trait GeoBoundaryCheckTrait
{
    ...
    public function formatBoundaries($boundaries)
    {
        ...
        $boundaries = collect($boundaries)->map(function ($boundary) {
            ...
            $point->lat = $boundary->lat ?? $boundary[1];
            $point->lng = $boundary->lng ?? $boundary[0];
            return $point;
        });
        return $boundaries;
    }
    ...
    public function isMultiPolygon($boundaries)
    {
        ...
        // Check if it's an array of polygons (i.e. 3-level nested array)
        return isset($boundaries[0][0]) && is_array($boundaries[0][0]);
    }
