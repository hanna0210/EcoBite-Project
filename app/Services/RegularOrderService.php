...
class RegularOrderService
{
    ...
    public function multipleVendorOrder(Request $request)
    {
        ...
        rest of usual request body
        */

        // Single transaction for the entire process
        DB::beginTransaction();
        ...
        try {
            ...
            foreach ($request->data as $vendorOrderData) {
                ...
                // Determine whether to use old or new order process
                if (allowOldUnEncryptedOrder()) {
                    try {
                        $orderData = $vendorOrderData;
                        $order = $this->oldMultipleVendorOrder($orderData, $request);
                    } catch (Exception $oldMultipleCheckError) {
                        logger("Error trying to use old multiple checkout system", [$oldMultipleCheckError]);
                        $order = null; // Ensure order is null if old method fails
                    }
                }

                // If old method didn't work or isn't enabled, use new method
                ...
                // Set the order status and save it once
                $order->save();
                $order->setStatus($this->getNewOrderStatus($request));
                $createdOrders[] = $order;
            }
    ...
    public function oldMultipleVendorOrder($orderData, $request)
    {
        ...
        //enforece product data check first to avoid wahala
