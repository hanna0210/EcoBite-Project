...
trait VendorAttributeTrait
{
    ...
    {
        return $this->belongsToMany(Subscription::class, 'subscription_vendor');
    }
    ...
    public function scopeWithinDeliveryArea($query, $latitude, $longitude)
    {
        // Subquery 1: Get IDs of vendors with delivery zones
        $deliveryZoneVendorIds = Vendor::whereHas('delivery_zones')
            ->byDeliveryZone($latitude, $longitude)
            ->pluck('id');

        // Subquery 2: Get IDs of vendors without delivery zones
        $rangeVendorIds = Vendor::whereDoesntHave('delivery_zones')
            ->withInRange($latitude, $longitude)
            ->pluck('id');

        // Combine IDs
        $allValidIds = $deliveryZoneVendorIds->concat($rangeVendorIds);

        return $query->whereIn('id', $allValidIds);
    }
