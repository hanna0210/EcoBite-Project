    ...
    public function placeOrder(Request $request, $userId = null)
    {
        ...
        if (!empty($request->stops)) {
            foreach ($request->stops as $stop) {
                ...
                $orderStop = new OrderStop();
                $orderStop->order_id = $order->id;
                //save delivery address if not exists
                if (!isset($stop["id"]) && !isset($stop["stop_id"])) {
                    $orderStopDeliveryAddress = new DeliveryAddress();
                    $orderStopDeliveryAddress->fill($stop['delivery_address']);
                    $orderStopDeliveryAddress->user_id = \Auth::id();
                    $orderStopDeliveryAddress = DeliveryAddressService::saveOrUpdate($orderStopDeliveryAddress);
                    $orderStop->stop_id = $orderStopDeliveryAddress->id;
                } else {
