...
<div>
    ...
    <x-baseview title="{{ __('Items') }}" :showNew="true">
    ...
    <div x-data="{ open: @entangle('showCreate') }">
        <x-modal-lg confirmText="{{ __('Save') }}" action="save" :clickAway="false">
            <p class="text-xl font-semibold">{{ __('Create Item') }}</p>
            {{-- show all errors --}}
            <x-form-errors />
            {{-- vendor --}}
            ...
            {{-- categories --}}
            <x-select2 :options="[]" name="categoriesIDs" title="{{ __('Categories') }}" :multiple="true"
                id="newCategoriesSelect" width="100" :ignore="true" />

            <div class="{{ $this->vendor_id == null ? 'hidden' : 'block' }}">
                {{-- show menu if the vendor doesn't use subcategories --}}
                {{-- menu --}}
                <div class="{{ !($this->has_subcategories ?? false) ? 'block' : 'hidden' }}">
                    <x-select2 :options="[]" name="selectedMenuIds" title="{{ __('Menu') }}" :multiple="true"
                        id="newMenusSelect" width="100" :ignore="true" />
                </div>

                {{-- subcategories --}}
                <div class="{{ $this->has_subcategories ?? false ? 'block' : 'hidden' }}">
                    <x-select2 :options="[]" name="subCategoriesIDs" title="{{ __('Subcategories') }}"
                        :multiple="true" id="newSubCategoriesSelect" width="100" :ignore="true" />
                </div>
            </div>
            ...
            {{-- tags --}}
            <x-select2 :options="$this->tags ?? []" name="selectedTagIds" title="{{ __('Tags') }}" :multiple="true"
                id="newTagsSelect" width="100" :ignore="true" />

            {{-- brand --}}
            <x-label for="brand_id" title="{{ __('Brand') }}">
                <livewire:select.brand-select name="brand_id" placeholder="{{ __('Search brand') }}"
                    :searchable="true" />
            </x-label>
    ...
    <div x-data="{ open: @entangle('showEdit') }">
        <x-modal-lg confirmText="{{ __('Update') }}" action="update" :clickAway="false">
            <p class="text-xl font-semibold">{{ __('Update Item') }}</p>
            {{-- show all errors --}}
            <x-form-errors />
            {{-- vendor --}}
            <div class="{{ auth()->user()->hasRole('manager') ?? false ? 'block' : 'hidden' }}">
            ...
            {{-- categories --}}
            <x-select2 :options="[]" name="categoriesIDs" title="{{ __('Categories') }}" :multiple="true"
                id="editCategoriesSelect" width="100" :ignore="true" />

            <div class="{{ $this->vendor_id == null ? 'hidden' : 'block' }}">
                {{-- show menu if the vendor doesn't use subcategories --}}
                {{-- menu --}}
                <div class="{{ !($this->has_subcategories ?? false) ? 'block' : 'hidden' }}">
                    <x-select2 :options="[]" name="selectedMenuIds" title="{{ __('Menu') }}"
                        :multiple="true" id="editMenusSelect" width="100" :ignore="true" />
                </div>

                {{-- subcategories --}}
                <div class="{{ $this->has_subcategories ?? false ? 'block' : 'hidden' }}">
                    <x-select2 :options="[]" name="subCategoriesIDs" title="{{ __('Subcategories') }}"
                        :multiple="true" id="editSubCategoriesSelect" width="100" :ignore="true" />
                </div>
            </div>
            ...
            {{-- tags --}}
            <x-select2 :options="$this->tags ?? []" name="selectedTagIds" title="{{ __('Tags') }}" :multiple="true"
                id="editTagsSelect" width="100" :ignore="true" />
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <x-details.item title="{{ __('Categories') }}" text="">
                    @if ($selectedModel != null)
                        {{ implode(', ', $selectedModel->categories()->pluck('name')->toArray()) }}
                    @endif
                </x-details.item>
                @if ($selectedModel->vendor->has_sub_categories ?? false)
                    <x-details.item title="{{ __('Subcategories') }}" text="">
                        @if ($selectedModel != null)
                            {{ implode(', ', $selectedModel->sub_categories()->pluck('name')->toArray()) }}
                        @endif
                    </x-details.item>
                @else
