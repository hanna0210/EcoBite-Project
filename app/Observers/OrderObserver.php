...
class OrderObserver
{
    ...
    {
        //only delivered
        if (in_array($model->status, ['delivered', 'completed', 'successful'])) {
            //send mail
            try {
                $emailCCs = [];
                if ($model->vendor != null && $model->vendor->email != null) {
                    $emailCCs[] = $model->vendor->email;
                }
                ...
                logger("Mail Error Sending OrderUpdateMail", [$ex]);
                // logger("Mail Error");
            }
        }
    }
