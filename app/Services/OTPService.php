...
class OTPService
{
    ...
    public function sendOTP($phone, $message, $gateway = null, $user = null, $code = null): ?String
    {
        ...
        } else if ($enabledSmsGateway == "arkesel") {
            $apiKey = env("ARKESEL_AUTHKEY");
            $sender = env("ARKESEL_SENDER");
            $phone = str_ireplace(" ", "", $phone);
            $phone = str_ireplace("+", "", $phone);
            $url = "https://sms.arkesel.com/sms/api?action=send-sms&api_key=$apiKey&to=$phone&from=$sender&sms=" . urlencode($message) . "";
            $response = Http::get($url);
